...
let private steriliseImport (import: string) =
    import.Replace('@', '_').Replace('-', '_').Replace('.', '_').Replace('/', '_')

let propTemplate kind def = $"export %s{kind} %s{def};"

let refTemplate ref = $"""export * as %s{snd ref} from "%s{fst ref}";"""
...
let importTemplate import = $"""import * as %s{steriliseImport import} from "%s{import}";"""
...
let emitImports = List.map importTemplate >> String.concat "\n"
